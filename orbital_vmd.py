...
def build_multi_batch_render_tcl(
    render_items: Sequence[Mapping[str, object]],
    state_or_path: VmdViewState | Path | str,
    *,
    width: int | None = None,
    height: int | None = None,
    ...
    Every item must provide ``token``, ``cube_path`` and ``output_path``.
    Optional ``position`` and ``total`` values are included in stable progress
    markers consumed by the host application.  The existing single-image
    replay helpers remain authoritative, so batching changes process lifetime
    only and does not change any captured scene or renderer parameter.
    """

    if not render_items:
        raise OrbitalVmdValidationError("At least one orbital render item is required.")
    if renderer not in {"TachyonInternal", "Tachyon"}:
        raise OrbitalVmdValidationError("Only TachyonInternal or Tachyon is allowed.")
    if (native_state_path is None) != (reference_cube_path is None):
        raise OrbitalVmdValidationError(
    ...
    required_suffix = ".dat" if renderer == "Tachyon" else ".tga"
    normalized: list[tuple[str, Path, Path, int, int]] = []
    tokens: set[str] = set()
    outputs: set[Path] = set()
    default_total = len(render_items)
    for index, raw in enumerate(render_items, 1):
        token = str(raw.get("token") or "").strip()
        if not re.fullmatch(r"[A-Za-z0-9_.:-]+", token):
            raise OrbitalVmdValidationError("Orbital render token is invalid.")
        if token in tokens:
            raise OrbitalVmdValidationError("Orbital render tokens must be unique.")
        cube = Path(str(raw.get("cube_path") or "")).expanduser().resolve()
        ...
        if output in outputs:
            raise OrbitalVmdValidationError("Orbital render outputs must be unique.")
        try:
            position = int(raw.get("position", index))
            total = int(raw.get("total", default_total))
        except (TypeError, ValueError) as exc:
            raise OrbitalVmdValidationError(
                "Orbital render progress values are invalid."
            ) from exc
        if position < 1 or total < position:
            raise OrbitalVmdValidationError(
                "Orbital render progress values are out of range."
            )
        tokens.add(token)
        outputs.add(output)
        normalized.append((token, cube, output, position, total))

    lines = [
        "# Generated by orbital_vmd.py for one-process multi-orbital rendering.",
        "proc _mo_unhex {value} { return [encoding convertfrom utf-8 [binary format H* $value]] }",
    ]
    for token, cube, output, position, total in normalized:
        if native_state_path is not None:
            task_lines = _restore_native_state_tcl(
                state,
                cube,
                output,
                render_width,
        ...
        if task_lines and task_lines[0].startswith("proc _mo_unhex "):
            task_lines = task_lines[1:]
        if not task_lines or task_lines[-1] != "quit":
            raise OrbitalVmdValidationError("Generated orbital render task is incomplete.")
        task_lines = task_lines[:-1]
        lines.extend(
            [
                f'puts "MolecularStudio: orbital batch begin\\t{position}\\t{total}\\t{token}"',
                "set MO_TASK_CODE [catch {",
                *(f"    {line}" for line in task_lines),
                "} MO_TASK_MESSAGE MO_TASK_OPTIONS]",
                "if {$MO_TASK_CODE != 0} {",
                f'    puts stderr "MolecularStudio: orbital batch failed\\t{position}\\t{total}\\t{token}"',
                "    puts stderr $MO_TASK_MESSAGE",
                "    foreach MO_OLD_MOL [molinfo list] { catch {mol delete $MO_OLD_MOL} }",
                "    exit 1",
                "}",
                "foreach MO_OLD_MOL [molinfo list] { catch {mol delete $MO_OLD_MOL} }",
                f'puts "MolecularStudio: orbital batch done\\t{position}\\t{total}\\t{token}"',
            ]
        )
    lines.extend(
        [
            f'puts "MolecularStudio: orbital batch complete\\t{len(normalized)}"',
            "quit",
        ]
    )
    return "\n".join(lines) + "\n"
