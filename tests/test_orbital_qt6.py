    ...
    def test_friendly_selector_pairing_and_per_orbital_choice(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            try:
                ...
                self.assertTrue(settings["complete_odd_boundaries"])
                self.assertFalse(settings["keep_intermediate_files"])
                page.keep_intermediates_check.setChecked(True)
                self.assertTrue(page._settings()["keep_intermediate_files"])
