...
2. 通过 `HOMO 偏移` 到 `LUMO 偏移` 的紧凑范围选择轨道，也可输入手动表达式，并在解析结果中逐项取消。奇电子体系可自动补齐所选范围最低、最高轨道序号缺少的另一自旋轨道。
...
运行页面会显示当前阶段、参考进度、逐任务耗时和关键说明。若检测到某个所选轨道与其余轨道的能量差距异常大，软件会先提示检查并由用户确认是否继续。最终始终保留 PNG 与可编辑 SVG 能级图、单轨道图片和轨道能量 CSV；Cube 与用于复现、排查的过程文件可分别决定是否保留。修改视角或绘图失败时可以从相应阶段继续，不必重复生成全部 Cube。
