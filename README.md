...
2. 使用快捷选择、起止轨道范围或手动表达式选择轨道，也可在解析结果中逐项取消。
3. 选择正负相位绘图方案和轨道等值面。软件先生成参考 HOMO 并打开 VMD。
4. 在 VMD 中自由调整骨架、等值面、颜色、材质、灯光、视角等参数，完成后点击“保存全部参数并确认”。
5. 软件把最终 VMD 状态应用到所有轨道，以 Tachyon 统一渲染，并自动合成含能量、占据箭头和 α/β 通道的能级图。

结果包含完整能级图、单轨道图片、轨道能量 CSV、经过校验的视角状态、VMD 原生 Save State、运行清单和日志。修改视角或绘图失败时可以从相应阶段继续，不必重复生成全部 Cube。
