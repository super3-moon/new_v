...
运行页面会显示当前阶段、参考进度、逐任务耗时和关键说明。若检测到某个所选轨道与其余轨道的能量差距异常大，软件会先提示检查并由用户确认是否继续。结果同时包含 PNG 与可编辑 SVG 能级图、单轨道图片、轨道能量 CSV、经过校验的视角状态、VMD 原生 Save State、运行清单和日志。修改视角或绘图失败时可以从相应阶段继续，不必重复生成全部 Cube。
