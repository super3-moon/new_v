...
- **弱相互作用**：在同一入口选择 IRI、RDG/NCI 或 IGMH；IGMH 以分号分隔片段定义，并可用任务专属的 `IGMvdwscl=2.0` 设置加速片段间分析。可选生成与 Multiwfn/Gnuplot 脚本相同数据范围及 BGR 色标的填色散点图，无需另装 Gnuplot。指标等值面和颜色范围采用 Multiwfn 内置示例的科学参数，生成数据后可在 VMD 中自由调整完整场景，再由 Tachyon 渲染。
